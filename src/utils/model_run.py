import joblib
from utils.model_building.model_build import extract_train_and_test, grid_search_from_df, \
    find_best_hyperparams_and_features, find_best_model_and_further_info
...
## Train Model
######################################################################################################################
clean_past_df = extract_data_for_model(countries, train_seasons, needed_cols, data='Train')
...
grid, accuracy, pipeline = grid_search_from_df(train_df, test_df, lvl1_unneeded_cols, xgb_model, xgb_param_grid, 4,
                                               include_odds=True)
...
clean_upcoming_df = extract_data_for_model(countries, test_seasons, needed_cols)
...
ev_df.head()
get_overall_profit_of_past_predictions(clean_past_df, lvl1_unneeded_cols, best_model, acceptable_ev=0.05, use_probs=True,
                                       include_odds=True, stake=10)
get_overall_profit_of_past_predictions(clean_past_df, lvl1_unneeded_cols, best_model,
                                       include_odds=True, stake=10)
