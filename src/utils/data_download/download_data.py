...
def extract_links(html_path, suffix):
    ...
    links = [(a.get_text(strip=True), a["href"]) for a in soup.find_all("a", href=True) if a["href"].endswith(suffix)]
...
def combine_data_from_dir(base_dir, columns="All", export=False, export_filepath=None):
    ...
    df_list = []
    if len(csv_files) == 1:
        return move_full_csv_to_dir(csv_files[0], columns=columns, export=export, export_filepath=export_filepath)
