...
from utils.model_building.model_params import betting_company_maps, division_maps, prediction_maps
...
def get_average_profit_of_past_predictions(
    ...
    if use_probs:
        ...
        initial_profits_lst = [
            profit_of_probability_predictions(odds, predictions, actual, acceptable_ev)
            for odds, predictions, actual in zip(betting_vals_lst, y_prob_preds, list(y_test))
        ]
        profits_lst = list(filter(lambda x: x is not None, initial_profits_lst))
    else:
        profits_lst = [
            profit_of_prediction(odds, prediction, actual)
            for odds, prediction, actual in zip(betting_vals_lst, y_preds, list(y_test))
        ]
