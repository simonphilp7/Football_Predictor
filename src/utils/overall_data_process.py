...
from utils.data_download.download_data import get_recent_results, get_upcoming_matches
...
from utils.data_processing.final_clean import final_clean, predictions_col_names
...
def extract_data_for_model(countries, seasons, needed_cols, country_divisions, data="Preds"):
    ...
    if data == "Train":
        clean_past_df = add_all_features_to_df(clean_past_df, "data/team_locations.pkl", 20, 5)
        final_past_df = final_clean(clean_past_df, predictions_col_names)
        return final_past_df
    elif data == "Preds":
        clean_upcoming_df = clean_data(
            "", needed_cols, export=False, export_filepath=None, df=upcoming_df, test_data=True
        )
        ...
        clean_upcoming_df = add_all_features_to_df(
            clean_past_df, "data/team_locations.pkl", 20, 5, test_df=upcoming_filtered_df
        )
