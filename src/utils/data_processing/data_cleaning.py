...
def create_correct_bet_column_groups(df):
    ...
    correct_bet_column_groups = [
        [correct_bet_columns[idx], correct_bet_columns[idx + 1], correct_bet_columns[idx + 2]]
...
def extract_max_and_avg_odds(lst_of_odds):
    ...
    return pd.Series(
        [
            ...
            avg_draw_odds,
            avg_away_odds,
            round(max_home_odds, 2),
            round(max_draw_odds, 2),
...
def clean_data(path, needed_cols, export=False, export_filepath=None, df=None, test_data=False):
    ...
    ] = new_df.apply(lambda row: find_odds(row, groups), axis=1)
