...
def calculate_form_stats(df, team, games='All'):
    if games == 'All':
        ...
        if (home_df.empty) or (away_df.empty):
            raise TooFewGames('')
        no_of_home_games, prop_of_home_wins, prop_of_home_losses, avg_home_goals_scored, avg_home_goals_conceded = calculate_form_stats_for_home_or_away(
            home_df,
            'Home')
        no_of_away_games, prop_of_away_wins, prop_of_away_losses, avg_away_goals_scored, avg_away_goals_conceded = calculate_form_stats_for_home_or_away(
            away_df,
            'Away')
        prop_of_wins = round((prop_of_home_wins * (no_of_home_games / (no_of_home_games + no_of_away_games))) + (
                prop_of_away_wins * (no_of_away_games / (no_of_home_games + no_of_away_games))), 2)
        prop_of_losses = round((prop_of_home_losses * (no_of_home_games / (no_of_home_games + no_of_away_games))) + (
                prop_of_away_losses * (no_of_away_games / (no_of_home_games + no_of_away_games))), 2)
        avg_goals_scored = round(
            (avg_home_goals_scored * (no_of_home_games / (no_of_home_games + no_of_away_games))) + (
                    avg_away_goals_scored * (no_of_away_games / (no_of_home_games + no_of_away_games))), 2)
        avg_goals_conceded = round(
            (avg_home_goals_conceded * (no_of_home_games / (no_of_home_games + no_of_away_games))) + (
                    avg_away_goals_conceded * (no_of_away_games / (no_of_home_games + no_of_away_games))), 2)
    elif games == 'Home':
        no_of_games, prop_of_wins, prop_of_losses, avg_goals_scored, avg_goals_conceded = calculate_form_stats_for_home_or_away(
            df, 'Home')
    elif games == 'Away':
        no_of_games, prop_of_wins, prop_of_losses, avg_goals_scored, avg_goals_conceded = calculate_form_stats_for_home_or_away(
            df, 'Away')
...
def extract_form_stats(df, team, date, n, games='All'):
    ...
    try:
        prop_of_wins, prop_of_losses, avg_goals_scored, avg_goals_conceded = calculate_form_stats(filtered_df, team,
                                                                                                  games=games)
    except TooFewGames:
        return np.nan, np.nan, np.nan, np.nan, np.nan, np.nan
...
def extract_all_form_info(df, team, date, overall_n, home_and_away_n):
    col_names = ['_new_season', '_league_change', '_prop_of_wins', '_prop_of_losses', '_avg_goals_scored',
                 '_avg_goals_conceded']
...
def extract_all_match_info_dic(df, team_locations, home_team, away_team, date, overall_n, home_and_away_n):
    ...
    full_match_dic = all_home_team_info_dic | all_away_team_info_dic | {
        'away_team_dist_travelled': away_team_dist_travelled}
...
def extract_all_info_for_match(match_row, df, team_locations, overall_n, home_and_away_n):
    ...
    date = match_row['Date']
    full_match_dic = extract_all_match_info_dic(df, team_locations, home_team, away_team, date, overall_n,
                                                home_and_away_n)
...
def add_all_features_to_df(info_df, path_to_team_locations, overall_n, home_and_away_n, test_df=None):
    ...
    if test_df is None:
        ...
        new_df = test_df.apply(
            lambda row: extract_all_info_for_match(row, info_df, team_locations, overall_n, home_and_away_n), axis=1)
    needed_cols = ['Div', 'Date', 'HomeTeam', 'AwayTeam', 'FTHG', 'FTAG', 'FTR', 'HS', 'AS', 'HST', 'AST', 'HR', 'AR',
                   'AVG_BETH', 'AVG_BETD', 'AVG_BETA', 'MAX_BETH', 'MAX_BETD', 'MAX_BETA', 'MAX_BETH_COMP',
                   'MAX_BETD_COMP', 'MAX_BETA_COMP', 'Div_Name', 'Season']
...
# full_data_2_df = add_all_features_to_df(df, r"C:\Repos\Misc\Football_Predictor\data\team_locations.pkl",20, 5)
# full_data_2_df.to_csv('full_sample_set_2.csv',index=False)
